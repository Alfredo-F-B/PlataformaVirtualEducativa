...
namespace PlataformaVirutalEducativa.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl,Login datos)
        {
            if (ModelState.IsValid && WebSecurity.Login(model.UserName, model.Password, persistCookie: model.RememberMe))
            {
                datos.nick = model.UserName;
                datos.password = model.Password;
                UsuarioActions contexto = new UsuarioActions();
                    if (contexto.ActualizarEstado("Conectado", datos))
                    {
                        
                        Usuario user = contexto.getUsuario(datos);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult LogOff(Usuario user, Login datos, LoginModel model)
        {
            //if (Session["IdUsuario"] == null)
            //    return RedirectToAction("Error");
            int id = Convert.ToInt32(Session["IdUsuario"]);
            UsuarioActions contexto = new UsuarioActions();
            contexto.ActualizarEstado("Desconectado", id);
            WebSecurity.Logout();
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public ActionResult Register(RegisterModel model,Usuario user)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    user.Estado = "No Conectado";
                    int b = server.Usuario.OrderByDescending(a => a.Id).First().Id;
                    user.IdUserRoles = b + 1;
                    user.Nickname = model.UserName;
                    user.Password = model.Password;
                    user.Repass = model.Password;
                    user.FechaRegistro = DateTime.Now;
                    UsuarioActions contexto = new UsuarioActions();
                    if (!contexto.comprobarelnombredeusuario(user))
                    {
                        ViewBag.error = "El Nombre de Usuario ya esta siendo usado";
                        return View(user);
                    }
                    if (user.Password == user.Repass)
                    {
                        if (contexto.registrar(user))
                        {
                            if (contexto.ActualizarEstado("Conectado", user.Id))
                            {
                                Usuario usu = contexto.getUsuario(user.Id);
                                Session["IdUsuario"] = user.Id;
                                return RedirectToAction("Index", "Home");
                            }
                        }
